...
setwd("C:\\Users\\Daniel\\Dropbox\\Uni\\Honours\\Stats 781a - Project\\writeUp\\dissertation")
...
<<xtExample, results = "asis", echo = FALSE>>=
library(xtable)
words = rep(c("low", "medium", "high") ,3)
x = data.frame(var1 = rnorm(n = 9, mean = 10, sd = 2),
  			var2 = rnorm(n = 9, mean = 5, sd = 15),
				...
				var4 = rnorm(n = 9, mean = 0, sd = 100),
				factor = factor(sample(words)))
tbl = xtable(x, digits = 2, caption = "data frame of random data with booktabs option",
              label = "tbl:random")
align(tbl) = c("|lr|r|r|r|l|")
print(tbl, booktabs = TRUE)
@
...
<<tbl1, results = "asis", echo = FALSE>>=
source(file="R/xtv2.R")
tbl <- read.table("data/inputs.txt", sep="\t", header=TRUE, as.is = TRUE)
wrap(tbl, ticks = TRUE, lines = TRUE, rotate = TRUE, 
...
<<tbl2, results = "asis", echo = FALSE>>=
source(file="R/xtv4.R")
tbl <- read.table("data/latexfeatures.txt", sep="\t", header=TRUE, as.is = TRUE)
#tbl <- tbl[,1:9]
wrap(tbl, ticks = TRUE, lines = TRUE, rotate = TRUE,
...
<<tbl3, results = "asis", echo = FALSE>>=
source(file="R/xtv5.R")
tbl <- read.table("data/htmlfeatures.txt", sep="\t", header=TRUE, as.is = TRUE)
wrap(tbl, ticks = TRUE, lines = TRUE, rotate = TRUE,
...
\begin{figure}[h!]
<<ok, fig.align='center', fig.height=8, fig.width=8, echo = FALSE>>=
source(file = "R/draw.R")
@
...
library(png)
par(mar = rep(0, 4))
img <- readPNG("visio/originalsmall.png")
grid::grid.raster(img, height = 0.35)
@
\vspace{-110pt}
...
\begin{figure}[H]
\vspace{-80pt}
<<firstRefactor, echo = FALSE>>=
library(png)
par(mar = rep(0, 4))
img <- readPNG("visio/stage1small.png")
grid::grid.raster(img, width = 1, height = 0.5)
@
\vspace{-70pt}
...
library(png)
par(mar = rep(0, 4))
img <- readPNG("visio/processEDITsmall.png")
grid::grid.raster(img, width = 1)
@
\vspace{-100pt}
...
\begin{figure}[H]
%\vspace{-30pt}
<<final, echo = FALSE,fig.width = 7, fig.height = 10>>=
library(png)
par(mar = rep(0, 4))
img <- readPNG("visio/refactoredsmall.png")
